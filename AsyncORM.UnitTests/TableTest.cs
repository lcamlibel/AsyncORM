...
namespace AsyncORM.UnitTests
{
    ...
    public class TableTest
    {
        ...
        public async Task UpdateAsync_Sucess_WithPrimaryKey()
        {
            ...
            await
                table.UpdateAsync(
                    new {CustomerId = 2, CustomerName = "TestUpdate2", Address = "Address Update2", LocationId = 1},
                    setting);
        }
            ...
            await
                table.UpdateAsync(
                    new
                        {
                            CustomerId = 2,
                            CustomerName = "TestUpdate With Where3",
                            Address = "Address Update With Where3"
                        },
                    setting,
                    where);
        }
        ...
        {
            string connString = ConfigurationManager.ConnectionStrings["test"].ConnectionString;
            string where = "CustomerName like '%TestUpdat%'";
            var setting = new TableSetting
                              {
                                  TableName = "TestCustomer"
                              };
        ...
        {
            string connString = ConfigurationManager.ConnectionStrings["test"].ConnectionString;
            string where = "CustomerName like '%Henry%'";
            var setting = new TableSetting
                              {
                                  TableName = "TestCustomer"
                              };
        ...
        public async Task InsertAsync_Sucess_WithMultiplePrimaryKeyWithMultipleIsIdentityYTrue()
        {
            ...
            ITable table = new Table(connString);
            await
                table.InsertAsync(new {CustomerId = 1, CustomerName = "TestUpdate", Address = "Address Update"}, setting);
        }
    }
}